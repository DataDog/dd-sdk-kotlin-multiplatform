...
class ApplicationContextTestConfiguration<T : Context>(klass: Class<T>) :
    ...
    private fun mockPackageManager() {
        mockPackageManager = mock {
            on { getPackageInfo(fakePackageName, PackageManager.PackageInfoFlags.of(0)) } doReturn fakePackageInfo
            on { getPackageInfo(eq(fakePackageName), any<Int>()) } doReturn fakePackageInfo
        }
    }

    private fun mockAssetManager() {
        mockAssetManager = mock {
            on { open(any()) } doReturn fakeBuildId.byteInputStream()
        }
    }
