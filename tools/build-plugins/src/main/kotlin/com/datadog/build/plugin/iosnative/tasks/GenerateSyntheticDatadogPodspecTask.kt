...
abstract class GenerateSyntheticDatadogPodspecTask : DefaultTask() {
    ...
    fun generate() {
        ...
        val iosPodspecFile = File(rootDirFile, "DatadogSyntheticIOSRoot.podspec")
        ...
        iosPodspecFile.writeText(
            """
            |Pod::Spec.new do |s|
            |  s.name = 'DatadogSyntheticIOSRoot'
        ...
        val tvosPodspecFile = File(rootDirFile, "DatadogSyntheticTVOSRoot.podspec")
        val tvosPodDependencies = tvosPods.get()
            .joinToString(separator = "\n") { "  s.dependency '$it', '${podVersion.get()}'" }
        tvosPodspecFile.writeText(
            """
            |Pod::Spec.new do |s|
            |  s.name = 'DatadogSyntheticTVOSRoot'
            |  s.version = '${syntheticPodVersion.get()}'
            |  s.summary = 'Synthetic Datadog pod to prebuild native dependencies for KMP.'
            |  s.homepage = 'https://www.datadoghq.com/'
            |  s.license = { :type => 'Apache-2.0' }
        ...
        File(rootDirFile, "Podfile").writeText(
            ...
            |
            |abstract_target 'DatadogSyntheticPods' do
            |  target 'DatadogSyntheticIOS' do
            |    platform :ios, '${iosDeploymentTarget.get()}'
            |    pod 'DatadogSyntheticIOSRoot', :path => '.'
            |  end
            |
            |  target 'DatadogSyntheticTVOS' do
            |    platform :tvos, '${tvosDeploymentTarget.get()}'
            |    pod 'DatadogSyntheticTVOSRoot', :path => '.'
            |  end
            |end
            |
